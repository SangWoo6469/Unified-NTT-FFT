// ntt256_mont_const.cu
// N=256 NTT (radix {2,4,8,16}) with constants (twiddle & m) in Montgomery domain.
// - Inputs are in normal domain (no encoding).
// - All multiplications are Montgomery: mont_mul(a, wR) -> a*w mod q.
// - Add/Sub: raw + / - (no reduction), as requested.
...
static constexpr uint32_t Q     = 8380417u; // modulus (odd)
...
__device__ __forceinline__ uint32_t sub_lazy(uint32_t a, uint32_t b){ return a - b; }

// Montgomery multiplication (32-bit radix, one-limb)
// result = a*b*R^{-1} mod q, R=2^32
__device__ __forceinline__ uint32_t mont_mul(uint32_t a, uint32_t b){
    uint64_t t  = (uint64_t)a * (uint64_t)b;           // a*b
    uint32_t m  = (uint32_t)t * d_nprime;              // low32(t) * n' mod 2^32
    uint64_t u  = (t + (uint64_t)m * (uint64_t)d_Q) >> 32;
...
// ================= drop-1 pre-twiddle (Montgomery mul) =================
...
void drop1_pretwiddle_parallel(const StageDesc& stg, int bfu, int lane){
    ...
    if (iA < R){
        ...
        uint32_t wR = stg.flat[off + (iA - 1)]; // Montgomery-encoded twiddle
        svec[idx] = mont_mul(svec[idx], wR);    // a * (wR) -> a*w
    }
    if (iB < R){
        int idx = base + j1 + iB * j;
        uint32_t wR = stg.flat[off + (iB - 1)];
        svec[idx] = mont_mul(svec[idx], wR);
    }
}
...
void r4_step2_twiddle(const StageDesc& stg, int bfu, int lane){
    if (lane == 1){
        ...
        svec[i3] = mont_mul(svec[i3], d_m4_omega);
    }
}
__device__ __forceinline__
...
void r8_step3_twiddle(const StageDesc& stg, int bfu, int lane){
    if (lane == 1){ int i=addr_loc(stg,bfu,3); svec[i] = mont_mul(svec[i], d_m8[3]); }
    if (lane == 3){ int i=addr_loc(stg,bfu,6); svec[i] = mont_mul(svec[i], d_m8[5]); }
    if (lane == 2){ int i=addr_loc(stg,bfu,5); svec[i] = mont_mul(svec[i], d_m8[6]); }
    if (lane == 0){ int i=addr_loc(stg,bfu,7); svec[i] = mont_mul(svec[i], d_m8[7]); }
}
__device__ __forceinline__
...
void r16_p4_tw(const StageDesc& stg, int bfu, int lane){
    switch(lane){
      case 0:{ uint32_t s0=s0_accum[bfu], s1=s1_accum[bfu];
               s0_accum[bfu]=mont_mul(s0,d_m16[4]);  // s0 * m[4]R
               s1_accum[bfu]=mont_mul(s1,d_m16[5]);  // s1 * m[5]R
               } break;
      case 1:{ int i=addr_loc(stg,bfu,9 ); svec[i]=mont_mul(svec[i], d_m16[0]); } break;
      case 2:{ int i=addr_loc(stg,bfu,15); svec[i]=mont_mul(svec[i], d_m16[1]); } break;
      case 3:{ int i=addr_loc(stg,bfu,11); svec[i]=mont_mul(svec[i], d_m16[2]); } break;
      case 4:{ int i=addr_loc(stg,bfu,13); svec[i]=mont_mul(svec[i], d_m16[3]); } break;
      case 5:{ int i=addr_loc(stg,bfu,3 ); svec[i]=mont_mul(svec[i], d_m16[6]); } break;
      case 6:{ int i=addr_loc(stg,bfu,12); svec[i]=mont_mul(svec[i], d_m16[7]); } break;
      case 7:{ int i=addr_loc(stg,bfu,10); svec[i]=mont_mul(svec[i], d_m16[8]);
               i=addr_loc(stg,bfu,14);     svec[i]=mont_mul(svec[i], d_m16[9]); } break;
    }
}
__device__ __forceinline__
void r16_p5(const StageDesc& stg, int bfu, int lane){
    if (lane==0){
        ...
        int i5=addr_loc(stg,bfu,5);  svec[i5]=mont_mul(svec[i5], d_m16[10]);
        int i7=addr_loc(stg,bfu,7);  svec[i7]=mont_mul(svec[i7], d_m16[11]);
        int i6=addr_loc(stg,bfu,6);  svec[i6]=mont_mul(svec[i6], d_m16[12]);
    }
    ...
    int numStages)
{
    ...
    // bit-reverse load (2개씩), 입력은 일반 도메인 그대로
    ...
    // 출력도 일반 도메인 (곱셈에서만 mont_mul 사용했으므로 이미 mod q 결과)
    A[i0] = svec[i0];
    A[i1] = svec[i1];
}
...
// q^{-1} mod 2^32 (Newton iteration), returns inv = q^{-1} mod 2^32
static uint32_t inv_mod2_32(uint32_t q){
    // q is odd; start with 1 and iterate: inv <- inv*(2 - q*inv)
    uint32_t inv = 1;
    for (int i=0;i<5;i++){
        uint64_t t = (uint64_t)q * inv;
        inv = inv * (uint32_t)(2u - (uint32_t)t);
    }
    return inv;
}

// Montgomery encode constant on host: cR = c * R mod q, R = 2^32
static inline uint32_t mont_encode_const(uint32_t c, uint32_t q){
    // compute R mod q:
    uint64_t Rmod = ((uint64_t)1 << 32) % q;
    return (uint32_t)(( (uint64_t)c * Rmod ) % q);
}

// drop-1 twiddle (normal) -> then Montgomery-encode each entry
static std::vector<uint32_t> build_flat_stage_mont(int R, int j){
    int index = N / (R * j);
    uint32_t gp = modpowU(GAMMA, index, Q); // generator power (normal)
    ...
    for (int j1=0; j1<j; ++j1){
        ...
        for (int i=0;i<R;i++){
            ...
            t[i] = modpowU(gp, e, Q); // normal-domain twiddle
        }
        for (int i=0;i<R;i++)
            tb[ bitrev_host((unsigned)i, rev_bits) ] = t[i];
        for (int i=1;i<R;i++){
            uint32_t w = tb[i];                 // normal
            out[j1*(R-1) + (i-1)] = mont_encode_const(w, Q); // to Montgomery
        }
    }
    return out;
}

// m4/m8/m16 생성 (normal)-> Montgomery 인코드 후 업로드용 배열 채움
static void build_m4_mont(uint32_t omega4, uint32_t& out_mR){
    out_mR = mont_encode_const(omega4, Q);
}
static void build_m8_mont(uint32_t omega, uint32_t mR[8]){
    uint32_t inv2 = (Q + 1) / 2;
    uint32_t m[8]={0};
    ...
    for (int i=0;i<8;i++) mR[i] = mont_encode_const(m[i], Q);
}
static void build_m16_mont(uint32_t omega, uint32_t mR[13]){
    uint32_t inv2 = (Q + 1) / 2;
    ...
    m[9]  = dif(2,6);
    m[11] = dif(2,6);
    for (int i=0;i<13;i++) mR[i] = mont_encode_const(m[i], Q);
}
...
struct StagePack {
    ...
    std::vector<std::vector<uint32_t>> hflats; // Montgomery-encoded
...
static StagePack build_stagepack_for_plan(const std::vector<int>& plan){
    ...
    for (size_t s=0; s<plan.size(); ++s){
        int R = plan[s];
        P.hflats[s] = build_flat_stage_mont(R, j); // 이미 Montgomery-encoded
...
int main(int argc, char** argv){
    ...
    // 0) Montgomery params upload: n' and q
    {
        ...
        // n' = -q^{-1} mod 2^32
        uint32_t inv = inv_mod2_32(hQ);       // q^{-1} mod 2^32
        uint32_t hnprime = (uint32_t)(0u - inv);
        cudaMemcpyToSymbol(d_Q,      &hQ,      sizeof(uint32_t));
        cudaMemcpyToSymbol(d_nprime, &hnprime, sizeof(uint32_t));
    }

    // 1) BFU 내부 상수 업로드 (모두 Montgomery-encoded)
    {
        uint32_t om4R; build_m4_mont(OMEGA4, om4R);
        cudaMemcpyToSymbol(d_m4_omega, &om4R, sizeof(uint32_t));

        uint32_t m8R[8]; build_m8_mont(OMEGA8, m8R);
        cudaMemcpyToSymbol(d_m8, m8R, sizeof(m8R));

        uint32_t m16R[13]; build_m16_mont(OMEGA16, m16R);
        cudaMemcpyToSymbol(d_m16, m16R, sizeof(m16R));
    }
    ...
    // 4) 각 플랜마다 pretwiddle 사전 계산(몽고메리 인코드) -> runs회 커널 실행 시간 평균
    for (const auto& plan : plans){
        ...
        for (int r=0; r<runs; ++r){
            // 입력 생성(난수) & 업로드 (복사시간 제외) -- 입력은 일반 도메인
    ...
    printf("=== NTT N=256, all radix plans (Montgomery consts; factors in {2,4,8,16}, product=256) ===\n");
